      ...
      for (const consId of conseilleresIds) {
        ...
        if (saisiesCC && saisiesCC.length > 0) {
          if (saisiesCC.length === 1) {
            ...
            const ligneRef = saisiesCC.find(s => s.date_debut === dd && s.date_fin === df)
            if (ligneRef) {
              await supabase.from('saisies').update({ visites: visTotal, ventes: venTotal }).eq('id', ligneRef.id)
            } else {
