  ...
  async function handleSaisie() {
    ...
    if (!error) {
      ...
      for (const consId of conseilleresIds) {
        ...
        // Chercher la ligne CC exacte pour cette periode (meme date_debut et date_fin)
        const isPeriodeJour = dd === df  // saisie jour = date debut = date fin
        ...
        if (existSaisie) {
          ...
          await supabase.from('saisies').update({
            rdv: rdvFinal, visites: visFinal, ventes: venFinal,
          }).eq('id', existSaisie.id)
        } else {
          // Creer la ligne avec le bon type
          await supabase.from('saisies').insert({
            conseillere_id: consId,
            date: dd,
            date_debut: dd,
            date_fin: df,
            type_saisie: isPeriodeJour ? 'jour' : 'periode',
