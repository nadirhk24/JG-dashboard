...
import { syncCC, syncMarketing } from '../lib/sync'
...
async function injectData(parsed, fileType, modeInfo, dryRun = false, importId = null, fileName = '') {
  ...
  for (const row of parsed) {
    try {
      ...
      // ── Sync CC → Marketing pour TOUS les champs CC (Gap3 fix) ─────────────
      if (isCC && !dryRun) {
        await syncMarketing(row.date)
      }
      ...
      if (['visites_cc','ventes_cc'].includes(type)) {
        ...
        if (dryRun) {
          ...
          // Gap2 fix: sync flux_rdv → CC + Marketing
