  ...
  return (
    <>
      <aside style={{
        ...
        height: '100vh', background: '#2C2C2C',
        display: 'flex', flexDirection: 'column',
        zIndex: 100, overflowY: 'auto',
        transition: 'width 0.25s ease',
        overflowX: 'hidden'
      }}>
        <div style={{ padding: collapsed ? '20px 0' : '28px 24px 20px', borderBottom: '1px solid rgba(201,168,76,0.2)', display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'flex-start', gap: 12 }}>
          {!collapsed && (
            <div>
          ...
          {navItems.map((item, i) => {
            ...
            return (
              <NavLink key={item.path} to={item.path} title={collapsed ? item.label : ''} style={({ isActive }) => ({
                ...
                borderRight: collapsed && isActive ? '3px solid #C9A84C' : 'none',
        ...
        <div style={{ padding: collapsed ? '12px 0' : '12px 16px', borderTop: '1px solid rgba(201,168,76,0.15)', display: 'flex', justifyContent: collapsed ? 'center' : 'flex-end' }}>
          <button onClick={() => setCollapsed(p => !p)} style={{
            ...
            color: '#C9A84C', borderRadius: 8, padding: '6px 10px',
      ...
      <style>{`
        :root { --sidebar-width: ${collapsed ? '60px' : '230px'}; }
        main { margin-left: ${collapsed ? '60px' : '230px'} !important; transition: margin-left 0.25s ease; }
      `}</style>
    </>
  )
}
